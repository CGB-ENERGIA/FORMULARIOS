<template>
  <q-page class="q-pa-md">
    <div class="row q-col-gutter-md q-mb-md">
      <div class="col-12">
        <q-card flat bordered class="form-header-card">
          <q-card-section>
            <div class="text-h6 text-primary q-mb-md">
              Relação de Consumidores Ligados na Obra
            </div>

            <div class="section-title q-mb-sm">Informações da Obra</div>
            <div class="row q-col-gutter-md">
              <div class="col-12">
                <q-input
                  v-model="obra.descricaoObra"
                  label="Descrição da Obra"
                  outlined
                  dense
                />
              </div>
              <div class="col-12 col-md-6">
                <q-input
                  :model-value="FORNECEDOR_FIXO"
                  label="Fornecedor"
                  outlined
                  dense
                  readonly
                  filled
                />
              </div>
              <div class="col-12 col-md-6">
                <q-input v-model="obra.elementoPep" label="Elemento PEP" outlined dense />
              </div>
              <div class="col-12 col-md-6">
                <q-input
                  :model-value="obra.dataConclusao"
                  label="Data da Conclusão"
                  outlined
                  dense
                  mask="##/##/####"
                  placeholder="DD/MM/AAAA"
                  @update:model-value="onDataObraChange"
                >
                  <template #append>
                    <q-icon name="event" class="cursor-pointer">
                      <q-popup-proxy cover transition-show="scale" transition-hide="scale">
                        <q-date
                          :model-value="obra.dataConclusao"
                          mask="DD/MM/YYYY"
                          @update:model-value="onDataObraChange"
                        >
                          <div class="row items-center justify-end">
                            <q-btn v-close-popup label="Fechar" color="primary" flat />
                          </div>
                        </q-date>
                      </q-popup-proxy>
                    </q-icon>
                  </template>
                </q-input>
              </div>
              <div class="col-12 col-md-6">
                <q-input
                  :model-value="obra.dataEnergizacao"
                  label="Data da Energização"
                  outlined
                  dense
                  mask="##/##/####"
                  placeholder="DD/MM/AAAA"
                  @update:model-value="onDataObraChange"
                >
                  <template #append>
                    <q-icon name="event" class="cursor-pointer">
                      <q-popup-proxy cover transition-show="scale" transition-hide="scale">
                        <q-date
                          :model-value="obra.dataEnergizacao"
                          mask="DD/MM/YYYY"
                          @update:model-value="onDataObraChange"
                        >
                          <div class="row items-center justify-end">
                            <q-btn v-close-popup label="Fechar" color="primary" flat />
                          </div>
                        </q-date>
                      </q-popup-proxy>
                    </q-icon>
                  </template>
                </q-input>
              </div>
              <div class="col-12 col-md-6">
                <q-input
                  :model-value="TEC_OBRA_FIXO"
                  label="Téc da Obra"
                  outlined
                  dense
                  readonly
                  filled
                />
              </div>
              <div class="col-12 col-md-6">
                <q-input
                  :model-value="REGIONAL_FIXA"
                  label="Regional"
                  outlined
                  dense
                  readonly
                  filled
                />
              </div>
              <div class="col-12 col-md-6">
                <q-input v-model="obra.localidade" label="Localidade" outlined dense />
              </div>
              <div class="col-12 col-md-6">
                <q-input v-model="obra.municipio" label="Município" outlined dense />
              </div>
            </div>
          </q-card-section>
        </q-card>
      </div>
    </div>

    <div class="row items-center q-mb-sm q-gutter-sm">
      <div class="col section-title">Consumidores</div>
      <div class="col-auto q-gutter-sm">
        <q-btn color="primary" icon="add" label="Adicionar linha" outline @click="addConsumidor" />
        <q-btn
          color="positive"
          icon="download"
          label="Exportar Excel"
          @click="handleExport"
        />
        <q-btn
          color="secondary"
          icon="picture_as_pdf"
          label="Gerar PDF"
          @click="handleExportPdf"
        />
        <q-btn color="negative" icon="restart_alt" label="Limpar" outline @click="handleReset" />
      </div>
    </div>

    <q-table
      class="consumidores-table"
      flat
      bordered
      :rows="consumidores"
      :columns="columns"
      row-key="id"
      :pagination="{ rowsPerPage: 0 }"
      hide-pagination
      virtual-scroll
      :virtual-scroll-item-size="56"
      style="max-height: 70vh"
    >
      <template #header="props">
        <q-tr :props="props">
          <q-th rowspan="2" auto-width>Nº</q-th>
          <q-th rowspan="2" style="min-width: 180px">NOME</q-th>
          <q-th rowspan="2" style="min-width: 130px">NÚMERO DO MEDIDOR</q-th>
          <q-th colspan="3">TIPO DE LIGAÇÃO</q-th>
          <q-th colspan="5">PADRÃO</q-th>
          <q-th colspan="2">RAMAL DUPLEX (M)</q-th>
          <q-th colspan="2">RAMAL TRIPLEX (M)</q-th>
          <q-th rowspan="2" style="min-width: 140px">POSTE DE LIGAÇÃO</q-th>
          <q-th rowspan="2" style="min-width: 120px">DATA LIGAÇÃO</q-th>
          <q-th rowspan="2" auto-width></q-th>
        </q-tr>
        <q-tr>
          <q-th>MO</q-th>
          <q-th>BI</q-th>
          <q-th>TRI</q-th>
          <q-th>5M</q-th>
          <q-th>7M</q-th>
          <q-th>CPP</q-th>
          <q-th>PADRÃO</q-th>
          <q-th>KIT</q-th>
          <q-th>PADRÃO</q-th>
          <q-th>KIT</q-th>
          <q-th>PADRÃO</q-th>
          <q-th>KIT</q-th>
        </q-tr>
      </template>

      <template #body="props">
        <q-tr :props="props">
          <q-td class="text-center">{{ props.row.id }}</q-td>
          <q-td>
            <q-input v-model="props.row.nome" dense outlined hide-bottom-space />
          </q-td>
          <q-td>
            <q-input v-model="props.row.numeroMedidor" dense outlined hide-bottom-space />
          </q-td>

          <q-td v-for="tipo in tiposLigacao" :key="'tl-' + tipo.value" class="text-center">
            <q-radio v-model="props.row.tipoLigacao" :val="tipo.value" dense />
          </q-td>

          <q-td v-for="pad in padroes" :key="'pd-' + pad.value" class="text-center">
            <q-radio v-model="props.row.padrao" :val="pad.value" dense />
          </q-td>

          <q-td v-for="ramal in ramalOpcoes" :key="'rd-' + ramal.value" class="text-center">
            <q-radio v-model="props.row.ramalDuplex" :val="ramal.value" dense />
          </q-td>

          <q-td v-for="ramal in ramalOpcoes" :key="'rt-' + ramal.value" class="text-center">
            <q-radio v-model="props.row.ramalTriplex" :val="ramal.value" dense />
          </q-td>

          <q-td>
            <q-input
              v-model="props.row.posteLigacao"
              dense
              outlined
              hide-bottom-space
              placeholder="PG ou coordenada"
            />
          </q-td>
          <q-td>
            <q-input
              :model-value="obra.dataEnergizacao || obra.dataConclusao"
              dense
              outlined
              hide-bottom-space
              readonly
              filled
            />
          </q-td>
          <q-td class="text-center">
            <q-btn
              flat
              round
              dense
              color="negative"
              icon="delete"
              @click="removeConsumidor(props.rowIndex)"
            >
              <q-tooltip>Remover linha</q-tooltip>
            </q-btn>
          </q-td>
        </q-tr>
      </template>
    </q-table>
  </q-page>
</template>

<script setup lang="ts">
import { storeToRefs } from 'pinia';
import { useQuasar } from 'quasar';
import type { QTableColumn } from 'quasar';
import { useConsumidoresStore, FORNECEDOR_FIXO, REGIONAL_FIXA, TEC_OBRA_FIXO } from 'src/stores/consumidores';
import { consumidorPreenchido, exportToExcel } from 'src/utils/excel';
import { exportToPdf } from 'src/utils/pdf';

const $q = useQuasar();
const store = useConsumidoresStore();
const { obra, consumidores } = storeToRefs(store);
const { addConsumidor, removeConsumidor, resetForm, syncDatas } = store;

function onDataObraChange(value: string | number | null) {
  syncDatas(String(value ?? ''));
}

const tiposLigacao = [
  { label: 'MO', value: 'MO' as const },
  { label: 'BI', value: 'BI' as const },
  { label: 'TRI', value: 'TRI' as const },
];

const padroes = [
  { label: '5M', value: '5M' as const },
  { label: '7M', value: '7M' as const },
  { label: 'CPP', value: 'CPP' as const },
  { label: 'PADRÃO', value: 'PADRAO' as const },
  { label: 'KIT', value: 'KIT' as const },
];

const ramalOpcoes = [
  { label: 'PADRÃO', value: 'PADRAO' as const },
  { label: 'KIT', value: 'KIT' as const },
];

const columns: QTableColumn[] = [{ name: 'id', label: 'Nº', field: 'id' }];

async function handleExport() {
  const preenchidos = consumidores.value.filter(consumidorPreenchido);
  if (preenchidos.length === 0) {
    $q.notify({
      type: 'warning',
      message: 'Preencha ao menos um consumidor antes de exportar.',
    });
    return;
  }

  try {
    const fileName = await exportToExcel(obra.value, consumidores.value);
    $q.notify({
      type: 'positive',
      message: `Arquivo ${fileName} gerado com sucesso.`,
    });
  } catch (error) {
    $q.notify({
      type: 'negative',
      message: 'Erro ao exportar Excel.',
    });
    console.error(error);
  }
}

async function handleExportPdf() {
  const preenchidos = consumidores.value.filter(consumidorPreenchido);
  if (preenchidos.length === 0) {
    $q.notify({
      type: 'warning',
      message: 'Preencha ao menos um consumidor antes de gerar o PDF.',
    });
    return;
  }

  const dismiss = $q.notify({
    type: 'ongoing',
    message: 'Gerando PDF...',
    timeout: 0,
  });

  try {
    const fileName = await exportToPdf(obra.value, consumidores.value);
    dismiss();
    $q.notify({
      type: 'positive',
      message: `Arquivo ${fileName} gerado com sucesso.`,
    });
  } catch (error) {
    dismiss();
    $q.notify({
      type: 'negative',
      message: error instanceof Error ? error.message : 'Erro ao gerar PDF.',
    });
    console.error(error);
  }
}

function handleReset() {
  $q.dialog({
    title: 'Limpar formulário',
    message: 'Deseja apagar todos os dados preenchidos?',
    cancel: true,
    persistent: true,
  }).onOk(() => {
    resetForm();
    $q.notify({ type: 'info', message: 'Formulário limpo.' });
  });
}
</script>
